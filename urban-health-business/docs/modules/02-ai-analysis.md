...
- 每批照片证据快照、状态、模型、requestId、usage和promptVersion元数据；
- 多批任务失败时禁止部分候选进入人工复核；
- 服务重启时复用已完成批次，只继续未完成批次；
- 原模型配置白名单。
...
- [x] 抽取超过20张的拆批、结果合并和候选去重为服务端纯函数；
- [x] 抽取逻辑不包含旧指标强制映射；
- [x] 将每批状态、照片证据快照和模型元数据写入AnalysisJob；
- [x] 建立源行为回归测试；
- [x] 完成多批成功、批次失败、重复候选和恢复续跑测试；
- [x] 通过全量单元测试和双服务隔离集成测试。

现有AnalysisJob、Candidate仓储、20张以内执行链路、结果规范化、风险统计和stale传播不重新实现。

下一A/B增量进入AB-06人工复核交互与标注图。
