...
| 能力 | 原版实现 | 等级 | 决策 |
|---|---|---:|---|
| 项目边界绘制 | 高德地图、地址定位、MouseTool多边形、面积和中心 | B | 抽取地图Provider |
| 范围内小区识别 | POI分页、点在面内、住宅过滤和去重 | B | 复用查询和过滤算法 |
...
| 能力 | 原版实现 | 等级 | 决策 |
|---|---|---:|---|
| 高德地图和控件 | 已实现 | B | 抽取Provider |
| 正向/反向地理编码 | 已实现 | B | 复用 |
| 地图点击定位 | 已实现 | B | 接入问题点位 |
| 半径Circle | 已实现 | B | 接入真实参数 |
| POI分类和分页查询 | 已实现 | B | 抽取 |
| POI允许和硬排除 | 已实现 | B | 抽取纯函数 |
| 名称、地址和空间去重 | 已实现 | B | 补测试后复用 |
| 原始POI持久化 | 原版只保存汇总 | C | 新增Business运行仓储 |
| 清洗人工确认 | 无确认记录 | C | 后续开发 |
| 坐标转换 | 无 | D | 后续开发 |
| 点位拖拽 | 无 | C | 后续开发 |
| 正式图层和政务GIS | 无 | D | 外部接入 |
