...
> 状态：本工作包A/B范围已完成（2026-07-29）。外业层级、任务创建/读取、Business任务工作台、任务引用恢复及本地/CloudBase存储契约均已接入；真实CloudBase环境验证仍依赖外部凭据。
...
> A/B范围实现状态：已完成。浏览器地图Controller、运行配置、BFF地址定位、GCJ-02绘制/回显、项目边界二次校验及缺配置状态均已接入；WGS84/GCJ-02转换仍属于C/D范围。
...
> A/B范围实现状态：已完成。BFF分页查询、分类规则、允许/硬排除、去重/近邻合并、住宅边界裁剪、原始POI及清洗快照均已写入Business SpatialAnalysisRun；逐条人工确认仍属于C/D范围。
...
> A/B范围实现状态：已完成（2026-07-29）。服务端任务执行器现已支持每20张自动拆批、跨批聚合、同图同分类候选去重、批次元数据持久化和恢复续跑；独立Worker、租约和单照片重试仍属于C/D范围。

### 11.1 当前基线

- Business已有持久化异步AnalysisJob、排队、失败、取消、重试和重启恢复；
- AB-05实施前单任务限制为最多20张照片；
...
AB-06已完成风险筛选、按当前筛选批量接受、BBox百分比预览、Canvas JPEG派生和标注图上传归档。派生上传会话保存分析、原图和候选来源；finalize在写正式问题前校验会话完整性，并把标注照片关联回分析、候选和OfficialIssue。标注失败保持`reviewing`以便重试，零接受候选仍可合法归档；Candidate revision、审计、幂等、归档只读和stale阻塞保持不变。

## 13. AB-07 报告快照与动态Renderer

> 状态：本工作包A/B范围已完成（2026-07-29）。新版本冻结项目、小区/楼栋、问题、分析、空间结果、原图与标注图来源；独立打印页按快照动态渲染章节、风险摘要、表格、画廊和来源索引。
...
> 状态：代码和Mock契约已完成（2026-07-29）。本地Provider仍为默认，CloudBase数据库/存储适配器、Collection配置和能力声明已建立；没有环境ID、权限和凭据，因此`productionVerified`保持`false`。
...
> 状态：本工作包A/B范围已完成（2026-07-29）。Business直接只读解析原412条资产，提供61指标、35问题分类、124问题类型和124整改建议的查询API及工作台目录，不复制公式、不生成分数。
