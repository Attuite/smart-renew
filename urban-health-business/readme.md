...
AB-00“主数据源与适配基础”已完成。ProjectData记录接口、外业任务和原Legacy迁移BFF也已接入；当前下一增量为SQLite转换/引用重建及Business正式问题、报告迁移。
...
`verify` 包含语法检查、92项单元测试、双服务隔离全过程集成测试、Demo 42文件完整性校验和原项目修改边界校验。集成测试只使用系统临时目录，不读写当前本地真实项目。
