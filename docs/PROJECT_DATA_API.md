...
项目数据索引用于统一管理项目档案、住宅台账、地理单元、照片、分析批次、问题、指标结果、报告和标准字典。每条数据都有稳定的 `id`，其他页面和外部系统可以通过该编号直接读取或建立引用。
...
`GET /api/project-data/{数据索引ID}`

这是其他页面和外部系统直接索引数据的标准接口。

### 保存标记或更新数据

`PUT /api/project-data/{数据索引ID}`

请求体为完整的数据索引对象。

### 删除单条索引数据

`DELETE /api/project-data/{数据索引ID}`
